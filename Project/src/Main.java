...
public class Main {
    ...
    public static void Calculate_Lowest_Distance(ArrayList<Edge>[] graph, String Start_District, String End_District) {
        ...
        // Store district names from graph
        for (int i = 0; i < graph.length; i++) {
            if (graph[i].size() > 0) {
                districtNames[i] = graph[i].get(0).district_name.toUpperCase();
            }
        }
        ...
        for (int i = 0; i < districtNames.length; i++) {
            if (districtNames[i] != null && districtNames[i].equals(Start_District.toUpperCase())) {
                startIndex = i;
            }
            if (districtNames[i] != null && districtNames[i].equals(End_District.toUpperCase())) {
                endIndex = i;
            }
        }
        ...
        for (int count = 0; count < graph.length - 1; count++) {
            ...
            for (Edge edge : graph[u]) {
                ...
                int neighborIndex = -1;
                for (int i = 0; i < districtNames.length; i++) {
                    if (districtNames[i] != null && districtNames[i].equals(edge.neighbor_district.toUpperCase())) {
                        neighborIndex = i;
                        break;
                    }
                }
        ...
        for (int i = pathLength - 1; i >= 0; i--) {
            ...
            if (i > 0) {
                ...
                // ✅ Fixed: Use equalsIgnoreCase to handle case mismatch
                for (Edge edge : graph[currentNode]) {
                    if (edge.neighbor_district.equalsIgnoreCase(districtNames[nextNode])) {
                        segmentDistance = edge.distance;
                        break;
                    }
                }

                System.out.print(" -> (" + segmentDistance + " km) -> ");
            }
        }

        // Estimate time assuming speed = 60 km/h
    ...
    public static void printAllDistrict(ArrayList<Edge>[] graph) {
        String[] districtNames = new String[graph.length];
        ...
        for (int i = 0; i < graph.length / 4; i++) {
            int idx1 = i;
            int idx2 = i + 16;
            int idx3 = i + 32;
            int idx4 = i + 48;

            System.out.printf("%-4d: %-15s", idx1 + 1, districtNames[idx1]);
            System.out.printf("%-4d: %-15s", idx2 + 1, districtNames[idx2]);
            System.out.printf("%-4d: %-15s", idx3 + 1, districtNames[idx3]);
            System.out.printf("%-4d: %-15s", idx4 + 1, districtNames[idx4]);
            System.out.println();
        }
    }
